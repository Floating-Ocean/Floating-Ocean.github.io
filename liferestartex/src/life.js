...
class Life {
    ...
    async dealChapterAgeData(chapterAgeList){ //a method to transform age data of each chapter.
        const chapterAge = new Array();
        for(const each in chapterAgeList){
            const {id, branch, backColorDark, backColorLight} = chapterAgeList[each];
            chapterAge.push([id, backColorDark, backColorLight, await json(`chapterAge_${id}`)])
        }
        return chapterAge;
    }
    ...
    getLastExtendTalent() { //这里为兼容性做了小适配
        let ext = this.#property.get(this.#property.TYPES.EXT);
        if(ext instanceof Array) return ext;
        return [ext];
    }
